...
import com.ebupt.android_playair.util.SpringSystem;
import com.facebook.rebound.BaseSpringSystem;
import com.facebook.rebound.SimpleSpringListener;
import com.facebook.rebound.Spring;
import com.nineoldandroids.view.ViewHelper;

@SuppressLint("ClickableViewAccessibility") public class JellyViewPager extends ViewPager {
	/**
	 * View最小缩放比例
	 */
    ...
    public interface ScrollPageToSendMessageListener{
        void sendMessage(String message);
    }
    //发消息接口
    private ScrollPageToSendMessageListener messageListener;
    ...
    public void setScrollPageToSendMessageListener(ScrollPageToSendMessageListener listener) {
	 ...
	 * 显示下一页
	 */
	public void showNext(boolean isReceiveMessage) {
		resetSpring();
		animOutIfNeeded(mHeight, 0,isReceiveMessage);
	}

	/**
	 * 显示上一页
	 */
	public void showPre(boolean isReceive) {
		resetSpring();
		animOutIfNeeded(-mHeight, 0,isReceive);
	}
	 ...
	 *            滑动速度
	 */
	private void animOutIfNeeded(float scrollDis, float velocityY,boolean isReceiveMessage) {
		float tranY = 0;
		...
		if (velocityY > FLING_VELOCITY || (scrollDis > OUT_DISTANCE_BOUDARY)) {
			if (hasNext()) {
                ...
                if(!isReceiveMessage)
                messageListener.sendMessage("down");
			} else {
				// 角度回正
				rotateSpring.setEndValue(0);
			}
			...
			if (hasPre()) {
				...
				rotateSpring.setAtRest();
                if(!isReceiveMessage)
                messageListener.sendMessage("up");
			} else {
				rotateSpring.setEndValue(0);
			}
		} else {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			if(Math.abs(distanceY) > mTouchSlop){
				...
				animOutIfNeeded(currentY - downY, velocityY,false);
				if (vTracker != null) {
					vTracker.recycle();
					vTracker = null;
				}
